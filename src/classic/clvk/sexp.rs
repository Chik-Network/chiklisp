...
pub fn to_sexp_type(allocator: &mut Allocator, value: CastableType) -> Result<NodePtr, EvalErr> {
    ...
    loop {
        ...
        match op {
                ...
                match top.borrow() {
                    ...
                    CastableType::TupleOf(left, right) => {
                        ...
                        match allocator.new_pair(NodePtr::NIL, NodePtr::NIL) {
                            Ok(pair) => {
                                stack.push(Rc::new(CastableType::CLVKObject(pair)));
                            }
                            Err(e) => {
                                return Err(e);
                            }
                    ...
                    CastableType::ListOf(_sel, v) => {
                        let target_index = stack.len();
                        stack.push(Rc::new(CastableType::CLVKObject(NodePtr::NIL)));
                    ...
                    CastableType::Bytes(b) => match allocator.new_atom(b.data()) {
                        Ok(a) => {
                            stack.push(Rc::new(CastableType::CLVKObject(a)));
                        }
                        Err(e) => {
                            return Err(e);
                        }
            ...
            SexpStackOp::OpSetPair(toset, target) => match top.borrow() {
                CastableType::CLVKObject(new_value) => match stack[target].borrow() {
                    CastableType::CLVKObject(target_value) => match allocator.sexp(*target_value) {
                        SExp::Pair(l, r) => {
                            if toset {
                                match allocator.new_pair(l, *new_value) {
                                    Ok(pair) => {
                                        stack[target] = Rc::new(CastableType::CLVKObject(pair));
                                    }
                                    Err(e) => {
                                        return Err(e);
                                    }
            ...
            SexpStackOp::OpPrepend(target) => match top.borrow() {
                CastableType::CLVKObject(f) => match stack[target].borrow() {
                    CastableType::CLVKObject(o) => match allocator.new_pair(*f, *o) {
                        Ok(pair) => {
                            stack[target] = Rc::new(CastableType::CLVKObject(pair));
                        }
                        Err(e) => {
    ...
    match stack.pop() {
        ...
        Some(top) => match top.borrow() {
            CastableType::CLVKObject(o) => Ok(*o),
...
// export class SExp implements CLVKType {
//   atom: Optional<Bytes> = None;
//   // this is always a 2-tuple of an object implementing the CLVK object protocol.
//   pair: Optional<Tuple<any, any>> = None;

//   static readonly TRUE: SExp = new SExp(new CLVKObject(Bytes.from("0x01", "hex")));
//   static readonly FALSE: SExp = new SExp(new CLVKObject(Bytes.NULL));
//   static readonly __NULL__: SExp = new SExp(new CLVKObject(Bytes.NULL));
