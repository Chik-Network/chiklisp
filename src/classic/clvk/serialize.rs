...
use crate::classic::clvk::as_rust::{TToSexpF, TValStack};
        ...
        match val_stack.pop().zip(item_b) {
            ...
            Some((l, r)) => {
                match to_sexp_f.invoke(allocator, CastableType::TupleOf(Rc::new(l), Rc::new(r))) {
                    Ok(c) => {
                        val_stack.push(CastableType::CLVKObject(c.1));
                        None
                    }
                    Err(e) => Some(e),
                }
...
impl OpStackEntry for OpReadSexp {
    fn invoke<'a>(
        ...
        match atom_from_stream(allocator, f, b, to_sexp_f) {
            Ok(v) => {
                val_stack.push(CastableType::CLVKObject(v));
                None
            }
            Err(e) => Some(e),
        }
...
pub struct SimpleCreateCLVKObject {}

impl<'a> TToSexpF<'a> for SimpleCreateCLVKObject {
