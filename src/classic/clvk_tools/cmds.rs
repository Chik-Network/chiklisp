...
use crate::classic::clvk::keyword_from_atom;
use crate::classic::clvk::serialize::{sexp_from_stream, sexp_to_stream, SimpleCreateCLVKObject};
use crate::classic::clvk::sexp::{enlist, proper_list, sexp_as_bin};
use crate::classic::clvk::OPERATORS_LATEST_VERSION;
use crate::classic::clvk_tools::binutils::{assemble_from_ir, disassemble, disassemble_with_kw};
use crate::classic::clvk_tools::clvkc::write_sym_output;
use crate::classic::clvk_tools::comp_input::{get_disassembly_ver, RunAndCompileInputData};
use crate::classic::clvk_tools::debug::check_unused;
use crate::classic::clvk_tools::debug::{
...
use crate::classic::clvk_tools::sha256tree::sha256tree;
...
use crate::compiler::clvk::start_step;
...
fn get_tool_description(tool_name: &str) -> Option<ConversionDesc> {
    if tool_name == "opc" {
        Some(ConversionDesc {
            desc: "Compile a clvk script.",
            conv: Box::new(OpcConversion {}),
        })
    } else if tool_name == "opd" {
        Some(ConversionDesc {
            desc: "Disassemble a compiled clvk script from hex.",
...
pub fn call_tool(
    ...
    parser.add_argument(
        ...
        Argument::new()
            ...
            .set_help("path to clvk script, or literal script".to_string()),
    );
...
pub fn cldb(args: &[String]) {
    ...
    let props = TArgumentParserProps {
        description: "Execute a clvk script.".to_string(),
        prog: format!("clvk_tools {tool_name}"),
    };
    ...
    parser.add_argument(
        ...
        Argument::new()
            .set_type(Rc::new(PathOrCodeConv {}))
            .set_help("filepath to clvk script, or a literal script".to_string()),
    );
    parser.add_argument(
        vec!["env".to_string()],
        Argument::new()
            ...
            .set_type(Rc::new(PathOrCodeConv {}))
            .set_help("clvk script environment, as clvk src, or hex".to_string()),
    );
...
pub fn launch_tool(stdout: &mut Stream, args: &[String], tool_name: &str, default_stage: u32) {
    ...
    // clvk_rs uses boxed callbacks with unspecified lifetimes so in order to
