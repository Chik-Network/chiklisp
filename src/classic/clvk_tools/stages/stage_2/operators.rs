...
use clvk_rs::chik_dialect::{ChikDialect, ClvkFlags};
...
use crate::classic::clvk_tools::binutils::{assemble_from_ir, disassemble_to_ir_with_kw};
use crate::classic::clvk_tools::ir::reader::read_ir;
use crate::classic::clvk_tools::ir::writer::write_ir_to_stream;
use crate::classic::clvk_tools::sha256tree::TreeHash;
...
use crate::classic::clvk_tools::stages::stage_2::compile::do_com_prog_for_dialect;
use crate::classic::clvk_tools::stages::stage_2::optimize::do_optimize;
...
pub struct CompilerOperatorsInternal {
    ...
    language_flags: u32,
    // Since flags have moved into the dialect in clvkr 0.17, they need to move
    // here too.
    flags: RefCell<ClvkFlags>,
}
...
// passed to clvkr, so I chose this way to mitigate the lack of specifiable lifetime as passing
...
impl CompilerOperatorsInternal {
    pub fn new(
        ...
        CompilerOperatorsInternal {
            ...
            language_flags,
            flags: RefCell::new(ClvkFlags::empty()),
        }
    }
...
impl Dialect for CompilerOperatorsInternal {
    ...
    fn op(
        ...
        let base_dialect = if new_operators > 0 {
            let rc: Box<dyn Dialect> = Box::new(ChikDialect::new(ClvkFlags::NO_UNKNOWN_OPS));
            rc
        } else {
            let rc: Box<dyn Dialect> = Box::new(OriginalDialect::new(ClvkFlags::NO_UNKNOWN_OPS));
            rc
        };
        ...
        let extensions_to_clvkr_during_compile = self.get_operators_extension();
            ...
            SExp::Atom => {
                ...
                if opbuf == b"_read" {
                    ...
                    base_dialect.op(
                        ...
                        sexp,
                        max_cost,
                        extensions_to_clvkr_during_compile,
                    )
                }
            }
            _ => base_dialect.op(
...
impl TRunProgram for CompilerOperatorsInternal {
    fn run_program(
        ...
        let new_flags = option
            ...
            .unwrap_or(ClvkFlags::NO_UNKNOWN_OPS | ClvkFlags::ENABLE_KECCAK_OPS_OUTSIDE_GUARD);
