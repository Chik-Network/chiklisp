...
use crate::compiler::preprocessor::{
    detect_chiklisp_module, parse_toplevel_mod, preprocess, Preprocessor, ToplevelModParseResult,
};
...
pub fn frontend(
    ...
    if let Some(_dialect) = detect_chiklisp_module(Srcloc::start(&opts.filename()), pre_forms)? {
        ...
        if output_forms.forms.is_empty() {
            return Err(CompileErr(
                ...
                "Module style chiklisp programs require at least one export".to_string(),
            ));
        }
