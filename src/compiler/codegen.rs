...
use crate::compiler::klvm::{run, sha256tree, truthy};
...
/// Compute a cache key for `helper`'s generated KLVM code inside `compiler`.
///
...
pub fn codegen(
    ...
    if matches!(
        ...
        //
        // The method is that we'll know the path to __chik__extras and write the
