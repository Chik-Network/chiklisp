...
pub fn detect_chiklisp_module(
    ...
    if pre_forms.is_empty() {
        ...
        // All other forms of chiklisp use a single form in a file.
        if pre_forms.len() > 1 {
...
#[test]
pub fn test_detect_chiklisp_module_classic() {
    ...
    assert!(detect_chiklisp_module(loc, &parsed).unwrap().is_none());
}
...
impl Preprocessor {
    ...
    fn import_program(
        ...
        let (have_module, dialect, classic_parse) =
            if let Some(dialect) = detect_chiklisp_module(Srcloc::start(filename), &pre_forms)? {
    ...
    fn find_macro(&mut self, loc: Srcloc, name: &[u8]) -> Result<Option<Rc<SExp>>, CompileErr> {
        ...
        let starting_program = CompileForm {
            loc: loc.clone(),
            args: Rc::new(SExp::Atom(loc.clone(), b"__chik__arg".to_vec())),
            include_forms: vec![],
