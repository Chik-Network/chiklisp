...
use crate::compiler::klvm::sha256tree_from_atom;
...
pub fn try_element_from_cache(
    ...
    let hex_file_name = format!(".chiklisp/{key}/{export_path}");
...
/// Exposes the cache-key segment used under `.chiklisp/<key>/` (tests and tooling only).
#[cfg(test)]
