...
/// KLVM, proving the cache must not be shared across dialect boundaries.
#[test]
...
/// once warm (pre-populated cache) -- must produce byte-identical KLVM output for every
...
fn funcache_cold_vs_warm_roundtrip() {
    ...
    for (key, cold_entry) in cold_cache.iter() {
        ...
        assert_eq!(
            ...
            "per-helper KLVM must match for {}",
            decode_string(&cold_entry.name)
        );
    }
}
