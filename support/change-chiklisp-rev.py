...
CHIKLISP_DEP_RE = re.compile(
    r'^(?P<prefix>\s*chiklisp\s*=\s*\{\s*git\s*=\s*"https://github\.com/Chik-Network/chiklisp\.git"\s*,\s*rev\s*=\s*")'
    r'(?P<rev>[A-Fa-f0-9]+)'
    r'(?P<suffix>"\s*\}\s*)$'
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Replace chiklisp git dependency rev values in matching lines."
        )
    )
    parser.add_argument(
        "new_rev",
...
def update_file(path: Path, new_rev: str) -> int:
    ...
    for line in original.splitlines(keepends=True):
        ...
        match = CHIKLISP_DEP_RE.match(stripped)
        if match is None:
