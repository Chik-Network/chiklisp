...
pip_installed_version=$(pip list | grep '^chiklisp\s' | awk '{print $2}')
python_import_version=$(python -c 'import chiklisp; print(chiklisp.get_version())')
...
if [ "$expected_version" == "$pip_installed_version" ] && [ "$expected_version" == "$python_import_version" ]; then
    ...
    echo "chiklisp VERSIONS does not match expected version"
